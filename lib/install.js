...
const { dirname, join, posixSep } = require('pathname')
const {
    readTextFileSync,
    writeFileSync,
    mergeFileSync,
...
function install(arg) {
    ...
    const script = JSON.parse(readTextFileSync(json_spec), null, 4)
    let version = null
    Object.keys(script)
        ...
        .forEach((e) => writeFileSync(join(repository_spec, 'package.json'), script[e].source, 'UTF-8N'))
