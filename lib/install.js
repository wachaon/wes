...
if (!isCLI(__filename) && !__callFromTest() && !exclusion)
    throw new Error('"install" can only be used on the command line')

const { NONE } = require('text')
const { security, has, unnamed, allow } = require('argv')
const ansi = require('ansi')

if (has('node') || has('n')) {
    if (!allow(security.unsafe))
...
const global_install = allow(security.unsafe) && (has('global') || has('g'))
const bare_install = allow(security.unsafe) && (has('bare') || has('b'))
...
function install(arg) {
    const { author, repository, script_raw, bundle_raw, module_dir, repository_spec, script_spec, json_spec } = genSpec(
        arg
    )
