...
function install(arg) {
    const { author, repository, script_raw, module_dir, repository_spec, script_spec, json_spec, package_spec, package_raw } = genSpec(arg)
    ...
    try {
        if (package_spec) {
            fs.download(package_raw, package_spec)
        }
    } catch (error) {}
    ...
    fs.moveFileSync(result_spec, script_spec)
