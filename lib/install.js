...
if (!isCLI(__filename) && !isCLI.__callFromTest()) throw new Error('"install" can only be used on the command line')
...
if (has('node') || has('n')) {
    if (!argv.allow(security.unsafe))
        throw new Error('If you specify "--node" or "-n", the security setting requires "--unsafe" or "--dangerous".')
}
...
function genSpec(arg) {
    ...
    const script_raw = `https://raw.githubusercontent.com/${author}/${repository}/master/${repository}.json${token != null ? token : NONE}`
    const bundle_raw = `https://raw.githubusercontent.com/${author}/${repository}/master/bundle.json${token != null ? token : NONE}`
...
function install(arg) {
    const { author, repository, script_raw, bundle_raw, module_dir, repository_spec, script_spec, json_spec } = genSpec(arg)
