...
const assert = function minitest_assert(assertion, description) {
    return assert.ok(assertion, description)
}
assert.ok = function minitest_assert_ok(assertion, description) {
    let res = typeof assertion === 'function' ? assertion() : assertion
    if (res !== true) throw new Error(res + description != null ? LF + description : '')
}
assert.ng = function minitest_assert_ng(assertion, description) {
    let res = typeof assertion === 'function' ? assertion() : assertion
    if (res !== false) throw new Error(res + description != null ? LF + description : '')
}
