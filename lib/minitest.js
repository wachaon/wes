...
const internal_equal = function minitest_internal_equal(recursive, base, target, strict) {
    // The first call clears the array to check for circular references.
    if (!recursive) {
        reference = []
        recursive = true
    }
...
const equal = function minitest_equal(base, target, strict) {
    let res = internal_equal(false, base, target, strict)
