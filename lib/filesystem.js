...
const exists = function filesystem_exists(spec, callback) {
    // (filespec: string): bool
    const err = FSO.FileExists(toWin32Sep(spec)) || FSO.FolderExists(spec)
    return callback != null ? err : callback(err)
}
