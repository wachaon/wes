...
const { toWin32Sep, split, absolute, relative, CurrentDirectory: cwd, resolve, toPosixSep } = require('pathname')
const { Type } = require('VBScript')
const chardet = require('chardet')
const genGUID = require('genGUID')
const http = require('Msxml2.XMLHTTP')

const { NONE, _FUNCTION } = require('text')
...
function isAuto(encode) {
    const enc = encode.toLowerCase()
    return enc === '_autodetect_all' || enc === '_autodetect' || enc === 'auto'
}

const readFileSync = function filesystem_readFileSync(filespec, encode) {
    // (filespec: string, encode?: string): string

    if (encode == null || encode === '') return new Buffer(readByteFileSync(filespec))
    return readTextFileSync(filespec, isAuto(encode) ? null : encode)
}
...
const convertEncoding = function filesystem_convertEncoding(buff, beforEncode, afterEncode) {
    var befor = beforEncode == null || beforEncode.toLowerCase() != 'auto' ? beforEncode : chardet.detect(buff)
    var text = BufferToText(buff, befor)

    var after = afterEncode || UTF_8
    try {
        ADODB.Open()
        ADODB.Type = AD_TYPE_TEXT
        ADODB.Position = 0
        ADODB.SetEOS()
        if (after.startsWith(UTF_8)) {
            ADODB.CharSet = UTF_8
            var bom = after.toUpperCase() === UTF_8BOM ? true : false
        } else ADODB.CharSet = after
        ADODB.WriteText(text)
        if (!bom) {
            ADODB.Position = 0
            ADODB.Type = AD_TYPE_BINARY
            ADODB.Position = 3
            var bytes = ADODB.Read()
            ADODB.Position = 0
            ADODB.SetEOS()
            ADODB.Write(bytes)
        }
        ADODB.Position = 0
        return Buffer.from(ADODB.Read())
    } catch (e) {
        e.message += `\n   (convertEncoding encode: ${beforEncode} => ${afterEncode})`
        throw e
    } finally {
        ADODB.Close()
    }
...
const existsFileSync = function filesystem_existsFileSync(filespec) {
    ...
    return FSO.FileExists(toWin32Sep(resolve(filespec)))
}

const existsdirSync = function filesystem_existsdirSync(dirspec) {
    // (dirspec: string): bool

    return FSO.FolderExists(toWin32Sep(resolve(dirspec)))
}

const existsSync = function filesystem_existsSync(filespec) {
    // (filespec: string): bool

    if (existsFileSync(filespec)) return 'file'
    if (existsdirSync(filespec)) return 'directory'
    return false
}
...
const exists = function filesystem_exists(filespec, callback) {
    ...
    return callback == null ? res : callback(res)
}
...
const mkdirsSync = function filesystem_mkdirsSync(spec) {
    ...
    dirs.reduce((acc, curr) => {
        const specs = resolve(cwd, acc, curr)
...
const readdirsSync = function filesystem_readdirsSync(spec, callback) {
    ...
    let files = new Enumerator(FSO.GetFolder(toWin32Sep(spec)).Files)
    let dirs = new Enumerator(FSO.GetFolder(toWin32Sep(spec)).SubFolders)

    if (typeof callback === _FUNCTION) {
        files
            .map((file) => toPosixSep(file.Path))
            .filter((path) => callback(path, null))
            .map((path) => {
                children.push(path)
                return path
            })
        dirs.map((dir) => toPosixSep(dir.Path)).map((path) => {
            if (callback(null, path)) children.push(path)
            children.push(readdirsSync(path, callback))
            return path
        })

        function flat(array, depth = Number.MAX_SAFE_INTEGER) {
            return array.reduce(function (acc, cur) {
                if (Array.isArray(cur)) {
                    Array.prototype.push.apply(acc, flat(cur, depth - 1))
                } else {
                    acc.push(cur)
                }
                return acc
