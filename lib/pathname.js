...
class Path {
    constructor(spec) {
        ...
        if (WORKING_DIRECTORY.test(spec)) {
            let [, root, body] = DRIVE.exec(CurrentDirectory)
            this.type = WORKING_DIRECTORY
            this.root = root.replace(/^([A-z])/, ($1) => $1.toUpperCase()) + posixSep
            this.body = body.replace(starts, NONE).replace(ends, NONE) + spec.replace(ends, NONE)
        } else if (UNC.test(spec)) {
...
const drivename = function pathname_drivename(path) {
...
const toNamespacedPath = function pathname_toNamespacedPath(path) {
    if (typeof path != 'string' || path.length === 0) return path

    const spec = resolve(path)

    if (spec.length <= 2) return path

    console.log(() => UNC.test(spec))
    console.log(() => DRIVE.test(spec))
    if (UNC.test(spec)) {
        const code = spec.charCodeAt(2)
        console.log(() => code)
        if (code !== 63 /* ? */ && code !== 46 /* . */) {
            return `//?/UNC/${spec.slice(2)}`
        }
    } else if (DRIVE.test(spec)) {
        return `//?/${spec}`
    }
...
const replaceExt = function pathname_replaceExt(spec, ext) {
    return spec.slice(0, extname(spec).length * -1) + ext
}
