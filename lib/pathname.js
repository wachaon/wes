...
const dirname = function pathname_dirname(path) {
    ...
    /*
    const _body = split(body)
    if (root != "") {
        if (_body.length === 0) return root
        if (_body.length === 1) return root
    } else {
        if (_body.length === 1) return body
    }
    return root + _body.slice(0, -1).join(posixSep)
    */
}
