...
const parse = function pathname_parse(path) {
    ...
    } else res = { type: RELATIVE, root: NONE, body: path }

    res.body = res.type == RELATIVE ? _normalize(res.body) : _normalize(res.body).replace(/^(\.\.\/)+/, NONE)
...
function bubbleUp(curr, matcher) {
    let res
    while (true) {
