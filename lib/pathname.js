...
class Path {
    constructor(spec) {
        ...
        this.body = this.type == RELATIVE ? _normalize(this.body) : _normalize(this.body).replace(parent, NONE)
    ...
    toString() {
        return [this.root, this.body].join(NONE)
    }
}

const parse = function pathname_parse(spec) {
