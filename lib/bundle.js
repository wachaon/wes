...
function bundle(...addFiles) {
    pipe()
        ...
        .filter((file) => file.Name.toUpperCase() === 'README.MD' || file.Name.toUpperCase() === 'LICENSE')
        .map((file) => resolve(file.ParentFolder.Path, file.Name))
        .process(
            readdirsSync(process.cwd(), (file) => file),
            (err, files) => {
                addFiles.push(...files)
            }
        )
