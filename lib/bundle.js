...
const { readTextFileSync, readFileSync, writeFileSync, existsFileSync } = require('filesystem')
...
if (isCLI(__filename)) {
    ...
    if (existsFileSync(pkg_spec)) {
        ...
        if ('main' in pkg) dest = resolve(WorkingDirectory, pkg.main)
    }
    if (dest === NONE) dest = resolve(WorkingDirectory, unnamed[1])
