...
function debug() {
    ...
    var message = format(arguments)
    var monotoneMessage = removeColor(message)
    if (argv.has('monotone')) WScript.StdOut.Write('DEBUG:' + SPACE + monotoneMessage + LF)
    else WScript.StdErr.Write(EIL + redBright + reverse + 'DEBUG:' + clear + SPACE + message + clear + LF)
    return monotoneMessage
}
...
function error() {
    var message = format(arguments)
    var monotoneMessage = removeColor(message)
    if (argv.has('monotone')) WScript.StdOut.Write('ERROR:' + SPACE + monotoneMessage + LF)
