...
var BUILD_DEBUG_COLOR = CYAN_BRIGHT
var INFO_COLOR = BLUE_BRIGHT
var ASSERT_COLOR = RED_BRIGHT
...
function info() {
    write([INFO_COLOR + REVERSE, 'info', REVERSE + CLEAR].join(SPACE) + SPACE, 0)
    return log.apply(null, Array.prototype.slice.call(arguments))
}

function warn() {
    write([WARN_COLOR + REVERSE, 'wran', REVERSE + CLEAR].join(SPACE) + SPACE, 0)
    return log.apply(null, Array.prototype.slice.call(arguments))
}

function debug() {
    if (!req('argv').has(DEBUG)) return void 0
    write([DEBUG_COLOR + REVERSE, 'debug', REVERSE + CLEAR].join(SPACE) + SPACE, 0)
    return log.apply(null, Array.prototype.slice.call(arguments))
}

function build_debug() {
    if (!req('argv').has(BUILD_DEBUG)) return void 0
    write([BUILD_DEBUG_COLOR + REVERSE, 'debug', REVERSE + CLEAR].join(SPACE) + SPACE, 0)
    return log.apply(null, Array.prototype.slice.call(arguments))
}
...
function assert(assertion) {
    ...
    if (!assertion) {
        console.print(ASSERT_COLOR + 'Assertion failed: ' + CLEAR)
        console.log.apply(null, objects)
    }
}
