...
const inspect = function inspect_inspect(value, opt) {
    ...
    if (isNumber(value)) return color.Number + String(value) + color.Clear
    if (isBoolean(value)) return color.Boolean + String(value) + color.Clear
    if (isSymbol(value)) {
        var symbolKey = Symbol.keyFor(value)
        if (!symbolKey) return color.Symbol + 'Symbol())' + color.Clear
        return color.Symbol + 'Symbol.for(' + inspect(Symbol.keyFor(value)) + color.Symbol + ')' + color.Clear
    }
    ...
    if (isBuffer(value)) {
        ...
        for (var k = 0; k < value.length; k++) {
            res.push('0x' + doubleDigit(value[k].toString(16)))
        }
        return color.Uint8Array + wrap(res.join(CAMMA), 'Buffer.from([', '])')
    }
    if (isArray(value)) {
        level++
        data.push(value)
    ...
    if (isWeakSet(value)) {
        ...
        var res = indent(opt) + '<items unknown>'
        level--
        return wrap(res + indent(opt), 'WeakSet {', '}')
    }
    if (isObject(value)) {
        data.push(value)
        level++
