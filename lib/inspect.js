...
const inspect = function inspect_inspect(value, opt) {
    ...
    if (typeof value === 'string') return color.String + toString(value) + clear
    if (typeof value === 'number') return color.Number + String(value) + clear
    if (typeof value === 'boolean') return color.Boolean + String(value) + clear
    if (toStringCall(value) === '[object Symbol]') return color.Symbol + 'symbol()' + clear
    ...
    if (typeof value === 'function') return color.Function + toFunctionString(value, opt) + clear
    if (toStringCall(value) === '[object Date]') return color.Date + toDateString(value) + clear
    if (toStringCall(value) === '[object RegExp]') return color.Regexp + String(value) + clear
