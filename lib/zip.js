...
function unzip(path, destinationPath, force = true) {
    ...
    const command = `powershell -Command Expand-Archive -Path "${path}" -DestinationPath "${dest}" ${force ? '-Force' : ''
        }`
...
function zip(path, destinationPath, force = true) {
    ...
    const command = `powershell -Command Compress-Archive -Path "${path}" -DestinationPath "${dest}" ${force ? '-Force' : ''
        }`
