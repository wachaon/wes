try {
    if ( !WScript.Arguments.Named.Exists( 'IE' ) ) {
        ...
        ie.PutProperty( 'host', this )
        WShell.Run( command, 6, true )
        ie.Quit()
        ...
        var ScriptControl = WScript.CreateObject( 'ScriptControl' )
        ScriptControl.Language = 'JScript'
        ScriptControl.AddCode( 'function eunme ( collection ) { return new Enumerator( collection ) }' )

        var hWndIE = WScript.Arguments.Named.Item( 'IE' )
        while ( typeof ie === 'undefined' ) {
            for ( var col = ScriptControl.Run( 'eunme', Shell.Windows() ); !col.atEnd(); col.moveNext() ) {
                if ( col.item().hWnd == hWndIE ) {
                    ie = col.item()
                    host = ie.GetProperty( 'host' )
                    break
                }
            }
        }
        ...
        function require( id, option ) {
            if ( graph[ id ] != null ) {
                ...
                function localRequire( name, option ) {
                    ...
                    if ( res == null ) return require( name, option && !option.startsWith( '_' ) ? option : null )
                    else return require( res, option && !option.startsWith( '_' ) ? option : null )
                }
                ...
                fn( localRequire, module, module.exports, console, graph[ id ].name || graph[ id ] );
                graph[ id ].code = fn
                stack.pop()
            ...
            try {
                if( option && option.startsWith( '_' ) ) return WScript.CreateObject( id, option )
            ...
            if ( id.startsWith( io.posixSep ) || id.startsWith( '.' + io.posixSep ) || id.startsWith( '..' + io.posixSep ) ) {
                points.push( io.join( curr, id ) )
            } else {
                points.push( io.join( curr, id ) )
                var hierarchy = io.split( curr )
                while ( hierarchy.length ) {
                    points.push( io.absolute( hierarchy.join( io.posixSep ) + io.posixSep + 'node_modules' + io.posixSep + id ) )
                    hierarchy.pop()
                }
            }
            var entry = null
            ...
            points.some( function( value ) {
                ...
                if ( ( res = value + '.js', io.fileExists( res ) ) ) return entry = res
                if ( ( res = value + '.json', io.fileExists( res ) ) ) return entry = res
                if ( ( res = value + '/index.js', io.fileExists( res ) ) ) return entry = res
                if ( ( package = value + '/package.json', io.fileExists( package ) ) ) {
                    var temp = JSON.parse( io.readFileSync( package, option && !option.startsWith( '_' ) ? option : null ) ).main
                    if ( temp != null ) {
                        if ( ( res = io.join( value, temp ), io.fileExists( res ) ) ) return entry = res
                        else if ( ( res = io.join( value, temp + '.js' ), io.fileExists( res ) ) ) return entry = res
                    }
                }
            } )
            if ( entry == null ) throw new Error( "module not found\ncaller: '" + curr + "' => require: '" + id + "'" )

            var loaded = history.find( function( val ) { return val[ 0 ] === entry } )
            ...
            graph[ uuid ] = {
                source: io.readFileSync( entry, option && !option.startsWith( '_' ) ? option : null ).replace( /\r/g, '' ),
                name: entry.match(/([^\/]+)$/)[0] + '',
                mapping: {}
            }
