...
*wes* — это консольная платформа, которая запускает *ECMAScript* на *WSH (Windows Script Host)* . Оригинальный текст *README* — [*japanese*](/README.md) . За исключением японского, это предложение с машинным переводом.  
...
*minitest* может писать простые тесты. Возвращаясь к базовой концепции версии `0.10.71` , мы сократили количество типов утверждений до трех.
...
| Парам     | Тип                   | Описание                                                                |
| :-------- | :-------------------- | :---------------------------------------------------------------------- |
| `value`   | `{Function\|Boolean}` | Функция, которая возвращает логическое значение или логическое значение |
...
| :--------- | :------------------------ | :---------------------------------------------------------------------------------------- |
| `value`    | `{Error}`                 | ошибка                                                                                    |
| `expected` | `{Error\|String\|RegExp}` | Регулярное выражение, оценивающее *constructor* , *message* или *stack* ожидаемой ошибки. |
| `message`  | `{String}`                | Сообщение в случае неудачи                                                                |
