...
`WScript.Echo` использует *console* вместо *wes* и `WScript.StdErr.WriteLine` . Вывод символов на консоль с помощью `console.log` . Он также поддерживает форматированные строки. Выводит отформатированную строку с использованием оператора форматирования `%` .
...
Работа с файлами и каталогами. `readTextFileSync` автоматически угадывает кодировку файла и читает ее.
...
Выполнение `complete` функции, когда все очереди завершены или вызывается функция `stop()` .
