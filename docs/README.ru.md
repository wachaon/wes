...
Кроме того, у *wes* есть *REP* , поэтому, если вы запустите его только с помощью `wes` , вы сможете войти в скрипт напрямую.
...
*REP* принимает ввод сценария до тех пор, пока вы не введете две пустые строки. Вы также можете проверить выполнение примера скрипта в *README.md* с помощью *REP* .
...
1.  В одном *repository* можно опубликовать только один модуль
...
Это модуль, который отправляет запрос *web driver* , управляющему браузером. Встроен в *@wachaon/edge* . Как и *@wachaon/edge* , для работы браузера требуется *web driver* .
