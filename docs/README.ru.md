...
`--bare` может опустить аргумент `require` из `author@repository` в `repository` . `--global` делает установленный пакет доступным для всех сценариев. Вышеупомянутые параметры должны быть указаны одновременно с параметром безопасности *wes* `--unsafe` или `--dangerous` .
...
*@wachaon/fmt* более *prettier* упакован для *wes* и форматирует скрипт. Также, если *@wachaon/fmt* установлен и возникает *Syntax Error* , можно указать место ошибки.
