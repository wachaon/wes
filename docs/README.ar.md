...
*wes* هو إطار عمل لوحدة التحكم يقوم بتشغيل *ECMAScript* على *WSH (Windows Script Host)* .
...
أيضًا ، يحتوي *wes* على *REP* ، لذا إذا بدأت باستخدام `wes` فقط ، يمكنك إدخال البرنامج النصي مباشرة.
...
يفسر *Chakra* ، وهو محرك تنفيذ البرنامج النصي ، بناء الجملة مثل `imoprt` ، ولكن لا يمكن تنفيذه كما هو لأن طريقة المعالجة مثل `cscript` غير محددة. في *wes* ، من خلال إضافة *babel* إلى الوحدة المدمجة ، يتم تنفيذه أثناء التحويل التسلسلي إلى *es module* . نتيجة لذلك ، يتم تضخيم تكاليف المعالجة وملف *wes.js* كتكلفة.


الوحدات المكتوبة في *es module* يتم تحويلها أيضًا إلى `require()` ، لذلك من الممكن إجراء مكالمات *ActiveX* . ومع ذلك ، فإنه لا يدعم مواصفات ترميز ملف الوحدة النمطية في *es module* . تتم قراءتها جميعًا عن طريق التخمين التلقائي.
...
أدخل وسيطات سطر الأوامر مع *REP* .
...
إذا كان `path` امتداد `.zip` ، تتم معالجة `unzip()` ولا يوجد وصف `.zip` . أو حتى إذا كان هناك امتداد `.zip` ، إذا كان هناك وصف لحرف بدل `*` ، فستتم معالجة `zip()` .
...
تستخدم لتثبيت حزمة *wes* المنشورة على *github* . من `version 0.10.28` ، سيتم تغيير مجلد التثبيت من `node_modules` إلى `wes_modules` . إذا كنت تقوم بالتثبيت على `node_modules` ، أضف خيار `--node` .
...
يمكن أن يحذف الخيار `--bare` الوسيطة `require` من `author@repository` إلى `repository` . يجعل الخيار `--global` الحزمة المثبتة متاحة لجميع البرامج النصية. يجب تحديد الخيارات المذكورة أعلاه في نفس الوقت الذي يتم فيه تحديد خيار أمان *wes* `--unsafe` أو `--dangerous` .
...
يمكن *install* تثبيت الحزم في مستودعات خاصة وكذلك الحزم في المستودعات العامة على *github* .


في *install* ، حدد الحزمة مع *@author/repository* . سيحاول التطبيق تنزيل عنوان url التالي.
...
يمكنك أيضًا تثبيت الحزم في مستودعات خاصة عن طريق تشغيلها في وحدة التحكم خلال عمر *token* .
...
*@wachaon/fmt* هي حزمة *prettier* لـ *wes* وتنسيق البرنامج النصي. أيضًا ، إذا تم تثبيت *@wachaon/fmt* *Syntax Error* يمكنك الإشارة إلى موقع الخطأ.
...
سيقوم هذا البرنامج النصي بإخراج *URL* التي تمت زيارتها بشكل تسلسلي إلى وحدة التحكم.
...
من خلال جعلها مدفوعة بالأحداث ، من الممكن التبديل بسهولة إلى التشغيل اليدوي من خلال عدم تعيين حدث للمعالجة يصعب التعامل معه باستخدام الطيار الآلي.
