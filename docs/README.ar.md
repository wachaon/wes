...
*wes* هو إطار عمل لتنفيذ *ECMAScript* على *WSH (Windows Script Host)* لوحدات التحكم.
...
يدعم *wes* أنظمة *commonjs module* التي تستخدم أنظمة الوحدات النمطية `require()` و *es module* العامة التي تستخدم `import` . ( *dynamic import* هو معالجة غير متزامنة ، لذلك فهو غير مدعوم)
...
يتم أيضًا نقل الوحدات النمطية الموصوفة بواسطة *es module* لتتطلب `require()` ، لذلك يمكن استدعاء *OLE* . ومع ذلك ، فإنه لا يدعم مواصفات ترميز ملف الوحدة النمطية. تتم قراءتها جميعًا عن طريق التخمين التلقائي.
...
يحتوي *wes* *built-in objects* مضمنة لا تحتوي عليها *WSH (JScript)* .
...
في *wes* ، تسمى حزمة من عدة وحدات *package* . يمكنك تثبيت *package* الخاصة بـ *wes* المنشورة على *github* . ستحتاج إلى *github repository* لنشر *package* . أيضًا ، يجب أن يكون اسم المستودع واسم الدليل المحلي متماثلين.


## *bundle*


عند نشر *package* على *github* ، قم بتجميع الوحدات النمطية المطلوبة في *bundle* وتغييرها إلى تنسيق يمكن استيراده عن طريق التثبيت.


لأسباب أمنية ، تقوم *bundle* بإنشاء ملف *.json* لأن *wes* لا يسمح لنا باستيراد *package* بتنسيق يمكن تنفيذه مباشرةً.
...
3.  يجب أن تكون حالة المستودع *public* عند نشر الحزمة.
...
تستخدم لتثبيت *package* *wes* المنشورة على *github* .
...
قم بفك ضغط الملف *zip* الذي تم تنزيله *msedgedriver.exe* إلى دليل العمل الخاص بك.
