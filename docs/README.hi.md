...
*wes* कंसोल के लिए *WSH (Windows Script Host)* पर *ECMAScript* निष्पादित करने के लिए एक ढांचा है।
...
-   `WScript.Quit` प्रोग्राम को बाधित नहीं कर सकता है और एक त्रुटि कोड नहीं देता है
...
*WScript.Shell* एक कार्यान्वयन के रूप में रनटाइम पर *wes* से `SendKeys` का उपयोग करता है। यदि निर्देशिका का पथ जहां *wes.js* सहेजा गया है, में *ascii* के अलावा अन्य वर्ण हैं, तो `SendKeys` कुंजी को सही ढंग से भेजने में सक्षम नहीं होगा और स्क्रिप्ट निष्पादित करने में सक्षम नहीं होगी।  
कृपया केवल *ascii* *wes.js* के सेव डेस्टिनेशन पाथ को कॉन्फिगर करें।


# कैसे इस्तेमाल करे


कंसोल में, उस फ़ाइल को निर्दिष्ट करें जो `wes` के बाद प्रोग्राम का शुरुआती बिंदु होगा। स्क्रिप्ट एक्सटेंशन *.js* को छोड़ा जा सकता है।
...
साथ ही, *wes* का *REPL* है, इसलिए यदि आप इसे केवल `wes` से प्रारंभ करते हैं, तो आप सीधे स्क्रिप्ट दर्ज कर सकते हैं।
...
लिपियों को तब तक स्वीकार किया जाएगा जब तक आप दो रिक्त पंक्तियाँ दर्ज नहीं करते। आप *REPL* के साथ *README.md* में नमूना स्क्रिप्ट के निष्पादन की जांच भी कर सकते हैं।
...
*wes* *commonjs module* सिस्टम का समर्थन करता है जो सामान्य `require()` और *es module* सिस्टम का उपयोग करते हैं जो `import` का उपयोग करते हैं। ( *dynamic import* अतुल्यकालिक प्रसंस्करण है, इसलिए यह समर्थित नहीं है)
...
आप आवश्यकता के साथ *require* `require('WScript.Shell')` जैसे *OLE* में भी आयात कर सकते हैं।
...
*Chakra* , जो स्क्रिप्ट का निष्पादन इंजन है, सिंटैक्स की व्याख्या करता है जैसे कि `imoprt` , लेकिन इसे निष्पादित नहीं किया जा सकता क्योंकि ऐसा इसलिए है क्योंकि `cscript` के रूप में प्रसंस्करण विधि परिभाषित नहीं है। *wes* में, बिल्ट-इन मॉड्यूल में बेबेल जोड़कर, हम इसे *es module* में क्रमिक रूप से *babel* करते हुए निष्पादित कर रहे हैं। परिणामस्वरूप, प्रोसेसिंग ओवरहेड और *wes.js* फ़ाइल लागत के रूप में फूली हुई हैं।


*es module* द्वारा वर्णित मॉड्यूल को भी `require()` में स्थानांतरित किया जाता है, इसलिए *OLE* को कॉल किया जा सकता है। हालांकि, यह मॉड्यूल फ़ाइल एन्कोडिंग विनिर्देश का समर्थन नहीं करता है। सभी स्वचालित अनुमान लगाकर पढ़े जाते हैं।
...
आप `ansi.color()` और `ansi.bgColor()` के साथ अपने खुद के रंग भी बना सकते हैं। तर्क *RGB* जैसे `255, 165, 0` या *color code* जैसे `'#FFA500'` उपयोग करता है। यह `orange` जैसे *color name* का समर्थन नहीं करता है।
...
कंसोल तर्क प्राप्त करता है। `cscript.exe` का कंसोल तर्क `/` के साथ नामित तर्क की घोषणा करता है `--` जबकि *wes* - और `-` के साथ नामित तर्क घोषित करता है।


*argv.unnamed* और *argv.named* ने कंसोल तर्क के मान प्रकार को *String* *Number* *Boolean* में से किसी एक पर डाला।


*REPL* के साथ कंसोल तर्क दर्ज करें।
...
यदि आप स्क्रिप्ट इंजन को *Chakra* में बदलते हैं, तो आप *JScript* -विशिष्ट *Enumerator* आदि का उपयोग नहीं कर पाएंगे। बिल्ट-इन मॉड्यूल *JScript* उन्हें उपलब्ध कराता है। हालांकि, *Enumerator* एक *Enumerator object* के बजाय एक *Array* देता है।
...
वाइल्डकार्ड्स `*` को `zip(path, destinationPath)` के `path` में लिखा जा सकता है।


इसका उपयोग *CLI* (कंसोल इंटरफेस) और *module* दोनों के साथ किया जा सकता है।
...
यदि `path` में एक्सटेंशन `.zip` है, तो `unzip()` संसाधित हो गया है और एक्सटेंशन `.zip` का कोई विवरण नहीं है। या यहां तक ​​कि अगर कोई `.zip` एक्सटेंशन है, अगर वाइल्डकार्ड `*` का विवरण है, तो `zip()` संसाधित किया जाएगा।
...
*wes* में, कई मॉड्यूल के बंडल को *package* कहा जाता है। आप *github* पर प्रकाशित *wes* के लिए *package* स्थापित कर सकते हैं। *package* को प्रकाशित करने के लिए आपको एक *github repository* की आवश्यकता होगी। साथ ही, रिपॉजिटरी का नाम और स्थानीय निर्देशिका का नाम समान होना चाहिए।


## *bundle*


*github* पर एक *package* प्रकाशित करते समय, *bundle* आवश्यक मॉड्यूल को बंडल करता है और उन्हें एक प्रारूप में बदल देता है जिसे इंस्टॉलेशन द्वारा आयात किया जा सकता है।


सुरक्षा कारणों से, *bundle* एक *.json* फ़ाइल बनाता है क्योंकि *wes* हमें ऐसे प्रारूप में *package* आयात करने की अनुमति नहीं देता है जिसे सीधे निष्पादित किया जा सकता है।


पैकेजिंग के लिए कुछ शर्तें हैं।


1.  एक *repository* में केवल एक प्रकार का मॉड्यूल प्रकाशित किया जा सकता है।
2.  *github* पर रिपॉजिटरी का नाम और स्थानीय कार्यशील निर्देशिका का नाम समान होना चाहिए।
3.  पैकेज प्रकाशित करते समय रिपॉजिटरी की स्थिति *public* होनी चाहिए।
...
5.  *.json* फ़ाइल आपकी कार्यशील निर्देशिका में *directory_name.json* नाम से बनाई जाएगी। यदि फ़ाइल का नाम बदल दिया गया है या फ़ाइल को स्थानांतरित कर दिया गया है तो इसे स्थापित नहीं किया जा सकता है।
...
*github* पर प्रकाशित *wes* के *package* को स्थापित करने के लिए उपयोग किया जाता है।


### कैसे इस्तेमाल करे


प्रारूप `@author/repository` में *install* के लिए तर्क पास करें।
...
*install* न केवल *github* पर सार्वजनिक रिपॉजिटरी में मॉड्यूल में स्थापित किया जा सकता है, बल्कि निजी रिपॉजिटरी में भी स्थापित किया जा सकता है।


*install* में, `author@repository` के साथ मॉड्यूल निर्दिष्ट करें। कार्यान्वयन निम्नलिखित डाउनलोड करता है।
...
आप एक निजी रिपॉजिटरी में एक मॉड्यूल को *token* के जीवनकाल में कंसोल में चलाकर भी स्थापित कर सकते हैं।
...
*@wachaon/fmt* बंडल *prettier* और स्क्रिप्ट को प्रारूपित करता है। साथ ही, यदि @ वाचाओं *@wachaon/fmt* `SyntaxError` स्थापित होने पर सिंटैक्स त्रुटि होती है, तो आप त्रुटि स्थान का संकेत दे सकते हैं।
...
यदि कार्यशील निर्देशिका में *.prettierrc* (JSON प्रारूप) है, तो यह सेटिंग में दिखाई देगा। *fmt* का उपयोग *CLI* (कंसोल इंटरफ़ेस) और *module* दोनों के साथ किया जा सकता है।
...
### जब *module* के रूप में उपयोग किया जाता है
...
*Internet Explorer* 2022/6/15 के साथ समर्थन पूरा करेगा। परिणामस्वरूप, `require('InternetExplorer.Application')` साथ एप्लिकेशन को संचालित करना असंभव हो जाता है।
...
डाउनलोड की गई *zip* को अनज़िप करें और *msedgedriver.exe* को अपनी कार्यशील निर्देशिका में ले जाएँ।
