...
*README* এর মূল পাঠ্য [*japanese*](docs/README.ja.md) । জাপানি ব্যতীত, এটি একটি মেশিন-অনুবাদিত বাক্য।  
...
-   *ECMAScript2015* *Chakra* চালানোর জন্য *Chakra* হল *Windows Script Host* স্ক্রিপ্টিং ইঞ্জিন
-   যেহেতু 32bit *cscript.exe* কার্যকর করা হয়েছে, তাই 64bit পরিবেশের জন্য নির্দিষ্ট কোন সমস্যা নেই।
-   `require` সাথে মডিউল আমদানি করুন
-   আদর্শ আউটপুটে রঙিন অক্ষর আউটপুট
-   স্বয়ংক্রিয়ভাবে ফাইল এনকোডিং অনুমান
...
\-- `--safe` `--usual` `--unsafe` `--dangerous` এর বাস্তবায়ন অসম্পূর্ণ, কিন্তু নামযুক্ত আর্গুমেন্ট সংরক্ষিত।
...
এছাড়াও আপনি `ansi.color()` এবং `ansi.bgColor()` দিয়ে আপনার নিজস্ব রং তৈরি করতে পারেন। আর্গুমেন্ট *RGB* ব্যবহার করে যেমন `255, 165, 0` বা *color code* যেমন `'#FFA500'` । আপনি `orange` মতো *color name* ব্যবহার করতে পারবেন না।
...
`--bare` বিকল্পটি `author@repository` থেকে `repository` তে `require` আর্গুমেন্ট বাদ দিতে `require` । `--global` বিকল্পটি সমস্ত স্ক্রিপ্টের জন্য ইনস্টল করা মডিউলগুলি উপলব্ধ করে। উপরের বিকল্পগুলি অবশ্যই *wes* সুরক্ষা বিকল্পের সাথে একই সময়ে নির্দিষ্ট করা উচিত `--unsafe` বা `--dangerous` ।
