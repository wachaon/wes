...
*wes* হল কনসোলের জন্য *WSH (Windows Script Host)* এ *ECMAScript* কার্যকর করার একটি কাঠামো।
...
কনসোলে, `wes` এর পরে যে ফাইলটি প্রোগ্রামের সূচনা পয়েন্ট হবে তা নির্দিষ্ট করুন। স্ক্রিপ্ট এক্সটেনশন *.js* বাদ দেওয়া যেতে পারে.
...
wes সাধারণ `require()` এবং *es module* সিস্টেম ব্যবহার করে এমন *wes* *commonjs module* সিস্টেম সমর্থন করে যা `import` ব্যবহার করে। ( *dynamic import* অ্যাসিঙ্ক্রোনাস প্রসেসিং, তাই এটি সমর্থিত নয়)
...
*Chakra* , যা স্ক্রিপ্টের এক্সিকিউশন ইঞ্জিন, সিনট্যাক্সকে ব্যাখ্যা করে যেমন `imoprt` , কিন্তু এটি কার্যকর করা যায় না কারণ এটি `cscript` হিসাবে প্রক্রিয়াকরণ পদ্ধতি সংজ্ঞায়িত করা হয়নি। *wes* এ, অন্তর্নির্মিত মডিউলে *babel* যোগ করে, আমরা এটিকে সম্পাদন করছি যখন ক্রমানুসারে *es module* স্থানান্তরিত করছি। ফলস্বরূপ, প্রসেসিং ওভারহেড এবং *wes.js* ফাইল একটি খরচ হিসাবে ফুলে গেছে।


*es module* দ্বারা বর্ণিত মডিউলগুলিকে `require()` এর জন্য ট্রান্সপিল করা হয়, তাই *OLE* বলা যেতে পারে। যাইহোক, এটি মডিউল ফাইল এনকোডিং স্পেসিফিকেশন সমর্থন করে না। সব স্বয়ংক্রিয় অনুমান দ্বারা পড়া হয়.
...
কনসোল আর্গুমেন্ট পায়। `cscript.exe` এর কনসোল আর্গুমেন্ট `/` এর সাথে একটি নামযুক্ত যুক্তি ঘোষণা করে `--` যখন *wes* `-` এবং- এর সাথে একটি নামযুক্ত যুক্তি ঘোষণা করে।


*argv.unnamed* এবং *argv.named* কনসোল আর্গুমেন্টের মান টাইপকে *String* *Number* *Boolean* একটিতে কাস্ট করে।


*REPL* সহ কনসোল আর্গুমেন্টগুলি লিখুন।
...
এটি *CLI* (কনসোল ইন্টারফেস) এবং *module* উভয়ের সাথেই ব্যবহার করা যেতে পারে।
...
*wes* এ, বেশ কয়েকটি মডিউলের একটি বান্ডিলকে *package* বলা হয়। আপনি *github* এ প্রকাশিত *wes* এর জন্য *package* ইনস্টল করতে পারেন। *package* প্রকাশ করতে আপনার একটি *github repository* প্রয়োজন হবে। এছাড়াও, সংগ্রহস্থলের নাম এবং স্থানীয় ডিরেক্টরির নাম অবশ্যই একই হতে হবে।


## *bundle*


*github* এ একটি *package* প্রকাশ করার সময়, প্রয়োজনীয় মডিউলগুলিকে *bundle* করে এবং একটি বিন্যাসে পরিবর্তন করে যা ইনস্টলেশনের মাধ্যমে আমদানি করা যেতে পারে।


নিরাপত্তার কারণে, *bundle* একটি *.json* ফাইল তৈরি করে কারণ *wes* আমাদের *package* এমন একটি বিন্যাসে আমদানি করার অনুমতি দেয় না যা সরাসরি চালানো যেতে পারে।
...
3.  প্যাকেজ প্রকাশ করার সময় সংগ্রহস্থলের অবস্থা *public* হতে হবে।
...
*wes* প্রকাশিত *github* এর জন্য *package* ইনস্টল করতে ব্যবহৃত হয়।
...
যদি ওয়ার্কিং ডিরেক্টরিতে *.prettierrc* (JSON ফরম্যাট) থাকে তবে সেটি সেটিংসে প্রতিফলিত হবে। *fmt* *CLI* (কনসোল ইন্টারফেস) এবং *module* উভয়ের সাথে ব্যবহার করা যেতে পারে।
...
ডাউনলোড করা *msedgedriver.exe* *zip* কাজের ডিরেক্টরিতে সরান।
