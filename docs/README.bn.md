...
*wes* হল *WSH (Windows Script Host)* এ *ECMAScript* চালানোর জন্য একটি কনসোল ফ্রেমওয়ার্ক। *README* এর মূল [*japanese*](/README.md) জাপানি ভাষায় হবে। জাপানি ব্যতীত অন্য পাঠ্যগুলি মেশিন অনুবাদ করা হবে। \\ অন্যান্য ভাষার পাঠ্যের জন্য, অনুগ্রহ করে নীচে থেকে নির্বাচন করুন।
...
*WScript.Shell* একটি বাস্তবায়ন হিসাবে রানটাইমে `SendKeys` *wes* SendKeys ব্যবহার করে। যদি ডিরেক্টরির পাথ যেখানে *wes.js* সংরক্ষিত হয় সেখানে *ascii* ছাড়া অন্য অক্ষর থাকে, `SendKeys` সঠিকভাবে কী পাঠাতে পারে না এবং স্ক্রিপ্টটি চালানো যাবে না। \\ *wes.js* গন্তব্য পথ শুধুমাত্র *ascii* তে কনফিগার করা উচিত। আপনি যদি ইতিমধ্যে *wes* ডাউনলোড করে থাকেন তবে আপনি নিম্নলিখিত কমান্ডের মাধ্যমে এটি আপডেট করতে পারেন।
...
ফাইলটি নির্দিষ্ট করে কমান্ডটি অনুসরণ করে `wes` কীওয়ার্ডটি লিখুন যা কনসোলে প্রোগ্রামের শুরুর বিন্দু হবে। স্ক্রিপ্ট এক্সটেনশন *.js* বাদ দেওয়া যেতে পারে।
...
`module.exports` বরাদ্দ করে এবং `require()` কল করে মডিউল পরিচালনা করুন। পরম পাথ এবং `./` এবং `../` দিয়ে শুরু হওয়া আপেক্ষিক পাথগুলি ছাড়া অন্য পাথগুলি *wes_modules* ডিরেক্টরিতে এবং সুবিধামত *node_modules* ডিরেক্টরিতে মডিউলগুলি সন্ধান করে৷ *wes* 's `require()` স্বয়ংক্রিয়ভাবে মডিউল ফাইলের এনকোডিং অনুমান করে, কিন্তু আপনি দ্বিতীয় আর্গুমেন্টের সাথে এনকোডিং নির্দিষ্ট করতে পারেন যদি এটি সঠিকভাবে অনুমান না করে।
...
সদস্য সমতার জন্য বস্তুর তুলনা, রেফারেন্স দ্বারা নয়। \\ NaN `true` `NaN === NaN` `function (){} === function (){}` `/RegExp/g === /RegExp/g` বা `{one: {two: 2}} === {one: {two: 2}}` `[1,2,3] === [1,2,3]` ইত্যাদি। \\ ক্লাস (বস্তু) তুলনা করার সময়, তাদের অবশ্যই একই কনস্ট্রাক্টর বা সুপারক্লাস হতে হবে যার `actual` `expected` ।
...
যাচাই করুন যে ত্রুটিটি সঠিকভাবে নিক্ষেপ করা হচ্ছে। \\ একটি ত্রুটি সঠিক কিনা তা প্রত্যাশিত ত্রুটির *constructor* কিনা বা *message* সমান এবং রেগুলার এক্সপ্রেশন *stack* মূল্যায়ন পাস করে কিনা তা দ্বারা নির্ধারিত হয়।
...
4.  `version 0.12.0` দিয়ে শুরু করে, কার্যকারী ডিরেক্টরির উপরে একটি ডিরেক্টরিতে সরাসরি মডিউল লোড করা প্যাকেজগুলি বান্ডিল করা হবে না। উপরের ডিরেক্টরির *wes_modules* বা *node_modules* করা যেতে পারে।
