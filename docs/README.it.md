...
`WScript.StdOut.WriteLine` *wes* di `WScript.StdErr.WriteLine` per generare stringhe colorate. `WScript.Echo` e `WScript.StdOut.WriteLine` sono bloccati. `WScript.StdErr.WriteLine` o `console.log` .
...
Un'attività viene utilizzata per *task* periodicamente più processi.

### Utilizzo

Se il processo richiede molto tempo, sarebbe bello visualizzare l'avanzamento sulla console.
...
Elaborazione del registro. È possibile registrare ed elaborare più processi in parallelo.
...
Sospende un processo specifico.
...
Specifica i caratteri che vengono stampati sulla console. Cambia carattere a intervalli regolari.
